...
*   Копируем содержимое в **${HOME}/.ssh/ed25519\_smart-proxy.pub** в файл **~/.ssh/authorized\_keys** на удаленном сервере вручную или командой
    ```bash
    $ ssh-copy-id ИМЯ_ПОЛЬЗОВАТЕЛЯ_ВАШЕГО_СЕРВЕРА@IP_АДРЕС_ВАШЕГО_СЕРВЕРА
    ```
...
если все ок, идем дальше, в противном случе разбираемся с SSH
...
*   Инициализируем
    ...
    $ ./smart-proxy init
    ```
Если у вас Linux, то после инициализации скрипт автоматически установится в `${HOME}/.local/bin/smart-proxy`, в противном случае необходимо самостоятельно установить скрипт в одну из директорий переменной `${PATH}`
...
*   Запускаем скрипт
    ...
    ```
*   Настраиваем PAC согласно инструкции [Настройка PAC](#pac-settings)
*   Радуемся жизни
...
|**IP\_CHECKER\_HOST**|ifconfig.me|Внешний хост для проверки работы прокси. При обращении должен возвращать IP адрес|
...
Ниже описаны инстукции для настройки PAC в браузерах и ОС
